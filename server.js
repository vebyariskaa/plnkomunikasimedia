...
app.post('/api/requests', upload.array('fotoDokumentasi', 50), async (req, res) => {
  try {
    ...
    const newRequest = {
      ...
      namaTerlibat: namaTerlibat || '',
      jabatanTerlibat: jabatanTerlibat || '',
      deskripsiKegiatan: finalDeskripsi,
      penyampaianStakeholder: penyampaianStakeholder || '',
      fotoPaths,
...
app.put('/api/requests/:id', requireAdmin, upload.array('fotoDokumentasi', 50), async (req, res) => {
  ...
  requests[idx] = {
    ...
    namaKegiatan,
    tanggalKegiatan,
    waktuKegiatan: waktuKegiatan !== undefined ? waktuKegiatan : requests[idx].waktuKegiatan || '',
    tanggalSelesai: tanggalSelesai || '',
